...
const HAVE_MONTSERRAT =
  fileExists(FONTS.regular) && fileExists(FONTS.medium) && fileExists(FONTS.bold);
...
function resolveSignaturePath(explicitPath?: string) {
  ...
  const candidates = ["assets/Sign.png", "assets/signature.png", "assets/sign.png"];
...
function hairline(doc: any, x1: number, y: number, x2: number, color = BRAND.line) {
...
function numberToWordsIndian(nInput: number) {
  ...
  const two = (num: number) => (num < 20 ? ones[num] : tens[Math.floor(num / 10)] + (num % 10 ? " " + ones[num % 10] : ""));
  const three = (num: number) => (num < 100 ? two(num) : ones[Math.floor(num / 100)] + " hundred" + (num % 100 ? " and " + two(num % 100) : ""));
  const parts: string[] = [];
...
function drawHeader(
  ...
  const logoSize = Math.max(64, Math.floor(BAND_H * 0.8));
  const logoX = P;
  const logoY = BAND_H - 4 - logoSize;
  if (logoBuf) {
    try {
  ...
  doc
    ...
    .fontSize(12)
    .fillColor("#FFFFFF")
    .text(meta?.no ? String(meta.no) : "—", innerX + labelW + tinyGap, baseY - 2, {
      width: innerW - (labelW + tinyGap),
    });
}

export async function buildInvoicePdfBuffer(payload: Partial<InvoicePayload>): Promise<Buffer> {
  const data: InvoicePayload = {
    ...
    items:
      payload.items && payload.items.length
        ? payload.items
        : [{ name: "Option Scalper PRO (Monthly)", qty: 1, rate: 14999, gst: 18, inclusive: true }],
    logo: payload.logo || path.join(process.cwd(), "assets", "Upholictech.png"),
    ...
    signature: (payload.signature || resolveSignaturePath(payload.signature)) as any,
    // signName: payload.signName || "Anil Kesi",
    // signTitle: payload.signTitle || "Founder and CEO",
    gstInclusive: !!payload.gstInclusive,
  } as InvoicePayload;

  const rawLogo = await loadImageBuffer(data.logo).catch(() => null);
  const logoBuf = rawLogo ? await removeSolidBackgroundToTransparent(rawLogo) : null;
  ...
  return await new Promise<Buffer>((resolve, reject) => {
    const doc = new PDFDocument({
      size: "A4",
      margin: 0,
      info: { Title: `${data.invoiceNo} — Invoice`, Author: data.fromName || "" },
    });
    ...
    if (data.billTo?.address) value(doc, data.billTo.address, col2X, y + 30, 260);
    if (data.billTo?.gstin) value(doc, `GSTIN: ${data.billTo.gstin}`, col2X, y + 48, 260);
    if (data.billTo?.email) value(doc, `Email: ${data.billTo.email}`, col2X, y + 68, 260);
    if (data.billTo?.phone) value(doc, `Phone: ${data.billTo.phone}`, col2X, y + 84, 260);
    ...
    if (data.shipTo && (data.shipTo.name || data.shipTo.address)) {
      ...
      if (data.shipTo.address) value(doc, data.shipTo.address, col3X, y + 30, 260);
    }
    ...
    const colX = { sr: tableX, desc: tableX + COLS.sr, total() { return this.desc + COLS.desc; } };

    const stroke = (x: number, y: number, w: number, h: number) =>
      doc.save().rect(x, y, w, h).lineWidth(0.8).strokeColor("#000").stroke().restore();

    const cell = (x: number, y: number, w: number, h: number, text?: string, opts: any = {}) => {
      stroke(x, y, w, h);
      doc
        .font(opts.bold ? "B" : "R")
    ...
    const headerH = 24;
    cell(colX.sr, tableY, COLS.sr, headerH, "Sr. No.", { bold: true, align: "center", noWrap: true });
    cell(colX.desc, tableY, COLS.desc, headerH, "Service Description", { bold: true, align: "center" });
    cell(colX.total(), tableY, COLS.total, headerH, "Total", { bold: true, align: "center", noWrap: true });
    tableY += headerH;
    ...
    for (let i = 0; i < data.items.length; i++) {
      ...
      cell(colX.sr, tableY, COLS.sr, rowH, String(sr++), { align: "center", noWrap: true });
      cell(colX.desc, tableY, COLS.desc, rowH, String(it.name || ""), { size: 10 });
      cell(colX.total(), tableY, COLS.total, rowH, money(calc.base), { align: "right", noWrap: true });
      tableY += rowH;
    }

    stroke(colX.sr, tableY, COLS.sr + COLS.desc, rowH);
    doc.font("B").fontSize(11).fillColor("#000").text("Total", colX.sr + 6, tableY + 6, {
    ...
    const summaryRow = (labelText: string, valueNum: number, bold = false) => {
      ...
      cell(colX.total(), tableY, COLS.total, h, money(valueNum), { bold, align: "right", noWrap: true });
      tableY += h;
    };
    ...
    const wordsHeight = Math.max(24, doc.heightOfString(words, { width: leftBlockW - 8 }) + 12);
    ...
    const maxW = 80, maxH = 60, topGap = 2;
    ...
    if (signatureBuf && sigW && sigH) {
      ...
      try {
        doc.image(signatureBuf, imgX, imgY, { width: renderW, height: renderH });
      } catch {}
    }
    ...
    doc.text(String(data.signName || ""), sigX, textY, { width: SIG_BLOCK_W, align: "right" });

    doc.font("R").fontSize(10).fillColor(BRAND.textMuted);
    const titleY = textY + nameLineHeight + 2;
    doc.text(String(data.signTitle || ""), sigX, titleY, { width: SIG_BLOCK_W, align: "right" });

    const thankYouY = Math.min(tableY + 24, sigY - 40);
    doc.font("M").fontSize(12).fillColor(BRAND.color)
       .text("Thank you for your business!", P, thankYouY, { align: "center", width: W - 2 * P });
    doc.font("R").fontSize(10).fillColor(BRAND.textMuted)
       .text("We look forward to serving you again.", P, thankYouY + 20, { align: "center", width: W - 2 * P });

    // existing page number (keep this)
    const pageNo = ((doc as any).page && (doc as any).page.number) || 1;
    doc.font("R").fontSize(9).fillColor("#9E9E9E").text(String(pageNo), W - P - 10, H - 28, {
      width: 10,
      align: "right",
    });

    // ✨ NEW: electronic document notice (centered footer)
    doc
      .font("R")
      .fontSize(9)
      .fillColor("#6B7280")
...
export async function sendInvoiceEmail(to: string, payload: InvoicePayload) {
  ...
  const text =
    payload.text ||
    `Dear ${payload.billTo?.name || "Customer"},\n\nPlease find attached your invoice ${invoiceNo}.\n\nRegards,\n${
      payload.fromName || BRAND.fromName
    }`;
  const html =
    payload.html ||
    `<p>Dear ${payload.billTo?.name || "Customer"},</p><p>Please find attached your invoice <b>${invoiceNo}</b>.</p><p>Regards,<br>${
      payload.fromName || BRAND.fromName
    }</p>`;
...
// OPTIONAL preview/send endpoints for testing
export function registerInvoiceRoutes(app: Express, basePath = "/api/invoice") {
  app.get(`${basePath}/preview`, async (_req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.post(`${basePath}`, async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  app.post(`${basePath}/send`, async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ ok: false, error: "failed_to_send_email", detail: e?.message || String(e) });
    }
  });
}        