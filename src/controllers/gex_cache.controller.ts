...
// /** Build tolerant expiry match on string/Date or strings-with-time */
...
//   const symContains = new RegExp("NIFTY", "i"); // allow NIFTY-50 / NIFTY50 etc
//   return {
//     $or: [
...
// /** Distinct expiries (YYYY-MM-DD) from any docs matching a loose filter */
...
//     if (d?._id) out.push(String(d._id));
//   }
//   return out;
// }
...
//   // Default to env NIFTY_LOT_SIZE (your .env has 75). Fallback to 50 if not set.
//   return typeof lot === "number" && lot > 0
//     ? lot
//     : (parseInt(process.env.NIFTY_LOT_SIZE || "75", 10) || 50);
// }

// /** ========== MAIN: GET /api/gex/nifty/cache?expiry=YYYY-MM-DD ========== */
...
//     // 0) If client passed expiry, try that first with loose base
//     if (requested) {
//       const f = { $and: [loose, expiryClause(requested)] };
//       doc = await coll.findOne(f, { sort: sortLatest });
//     }

//     // 1) If no doc and no requested expiry -> pick nearest in data
...
//     // 2) If still nothing, just take the newest doc for NIFTY-ish data
//     if (!doc) {
...
//     // 3) Still nothing? return helpful debug
//     if (!doc) {
...
//       // include raw totals too (non-breaking addition)
...
//     res.status(500).json({ error: "GEX_CACHE_FAILED", detail: String(e?.message || e) });
//   }
// };

// /** GET /api/gex/nifty/cache/expiries  — list expiries (loose) */
...
//     res.json({ symbol, securityId, seg, expiries });
//   } catch (e: any) {
//     res.status(500).json({ error: "EXPIRIES_LIST_FAILED", detail: String(e?.message || e) });
//   }
// };

// /** GET /api/gex/cache/debug  — quick peek of what's in option_chain */
...
//     res.json({ collection_count: count, latest_samples: sample, variants });
//   } catch (e: any) {
//     res.status(500).json({ error: "OC_DEBUG_FAILED", detail: String(e?.message || e) });
//   }
// };
