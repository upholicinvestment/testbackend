...
// Utility: Fast binary search for latest OI <= timestamp
function findLatestOI(arr: { timestamp: Date, OI: number }[], target: Date) {
  let left = 0, right = arr.length - 1, result: { timestamp: Date, OI: number } | null = null;
  while (left <= right) {
    const mid = Math.floor((left + right) / 2);
    const midTs = arr[mid].timestamp.getTime();
    if (midTs <= target.getTime()) {
      result = arr[mid];
      left = mid + 1;
    } else {
      right = mid - 1;
...
export default function registerNiftyRoutes(app: Express, db: Db) {
  // --- /api/nifty/atm-strikes-timeline (FAST) ---
  app.get('/api/nifty/atm-strikes-timeline', async (req: Request, res: Response) => {
    try {
      console.log('\n=== Processing ATM Strikes Timeline Request (FAST) ===');
      const intervalParam = req.query.interval as string || '3';
      const interval = parseInt(intervalParam, 10);
      const collection = db.collection('market_quotes');
      ...
      // Get all LTP records for Nifty for the date
      let docs = await collection.find(
        { security_id: 53216, timestamp: { $gte: todayStart, $lt: todayEnd } },
        { projection: { _id: 0, LTP: 1, timestamp: 1 } }
      ).sort({ timestamp: 1 }).toArray();

      if (!docs.length) {
        // Fallback to last available date
        const lastEntry = await collection.find({ security_id: 53216 }).sort({ timestamp: -1 }).limit(1).toArray();
        if (!lastEntry.length) {
          res.status(404).json({ error: 'No NIFTY data found' });
          return;
        }
        ...
        docs = await collection.find(
          { security_id: 53216, timestamp: { $gte: lastDate, $lt: lastEnd } },
          { projection: { _id: 0, LTP: 1, timestamp: 1 } }
        ).sort({ timestamp: 1 }).toArray();
      }
      ...
      const firstLTP = parseFloat(docs[0].LTP);
      const fixedATM = Math.round(firstLTP / 50) * 50;
      console.log(`Fixed ATM Strike for session: ${fixedATM} (first LTP = ${firstLTP})`);

      // --- Batch Load All Option Data for the Day (CE/PE) ---
      const allOptions = await collection.find(
        {
          strike_price: fixedATM,
          expiry_flag: "W",
          trading_symbol: { $regex: '^NIFTY-Jul2025' },
          option_type: { $in: ["CE", "PE"] },
          timestamp: { $gte: docs[0].timestamp, $lt: docs[docs.length - 1].timestamp }
        },
        { projection: { _id: 0, option_type: 1, OI: 1, timestamp: 1 } }
      ).sort({ timestamp: 1 }).toArray();

      // -- Fix: cast to correct types (no TS error)
      const ceOI: { timestamp: Date, OI: number }[] = [];
      const peOI: { timestamp: Date, OI: number }[] = [];
      for (const opt of allOptions) {
        if (opt.option_type === "CE" && opt.timestamp && typeof opt.OI !== "undefined") {
          ceOI.push({ timestamp: new Date(opt.timestamp), OI: Number(opt.OI) });
        } else if (opt.option_type === "PE" && opt.timestamp && typeof opt.OI !== "undefined") {
          peOI.push({ timestamp: new Date(opt.timestamp), OI: Number(opt.OI) });
        }
      }
      ...
      for (const doc of docs) {
        ...
        if (!intervalMap[roundedISO]) {
          ...
          const ce = findLatestOI(ceOI, rounded);
          const pe = findLatestOI(peOI, rounded);
          ...
          results.push({
            ...
            callOI: ce?.OI ?? null,
            putOI: pe?.OI ?? null,
            callTimestamp: ce?.timestamp ?? null,
            putTimestamp: pe?.timestamp ?? null
          });
        }
      }
  ...
  // You can add your other endpoints here...
}
