    ...
    try {
      ...
      if (r.status >= 500 && attempt < retries) {
        ...
        await new Promise((res) =>
          setTimeout(res, backoffMs * Math.pow(2, attempt - 1))
        );
        continue;
      }
...
const obParseTime = (x: any): number => {
  const s = String(
    x?.updatetime ?? x?.exchorderupdatetime ?? x?.exchtime ?? ""
  );
...
export function Orderbook(app: Express, _db: Db) {
  // Point to your Flask service (local)
  // const FLASK_BASE_URL = "http://127.0.0.1:5000";
  const FLASK_BASE_URL = "https://tech.upholictech.com";
  const WEBHOOK_SECRET = "KesiNgrokSecret2025";
  const TV_PREFIX = "TV_";
  ...
  const passthru =
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(getStatus(e))
          .json({ error: `${path.slice(1)}_failed`, detail: getDetail(e) });
      }
    };
  ...
  /** ---------- /api/summary : TODAY (IST) ALGO-only ---------- */
  type Summary = {
    totalPnl: number;
    totalTrades: number;
  ...
  app.get(
    ...
    (async (req: Request, res: Response) => {
      try {
        ...
        const obRaw = await fetchJson<any>(
          `${FLASK_BASE_URL}/angel/user/orderbook`,
          {
            timeoutMs: 10000,
            retries: 2,
            backoffMs: 250,
        ...
        const obTodayAlgo: OB[] = safeArray(obRaw)
          .filter((x: any) =>
            String(x?.ordertag ?? "").startsWith(TV_PREFIX)
          )
          .filter((x: any) => pickDateKeyFromOB(x) === todayKey)
          .map((x: any) => {
        ...
        const completeAlgoToday = obTodayAlgo
          .filter(
            (r) => r.status === "complete" && r.qty > 0 && Number.isFinite(r.price)
          )
          .sort((a, b) => a.t - b.t);
        ...
        const closeAgainst = (
          ...
          let remaining = qty;
          const matchFn = (l: Lot) =>
            incomingSide === "BUY" ? l.side === "SHORT" : l.side === "LONG";
          ...
          while (remaining > 0) {
            ...
            let slicePnl = 0;
            if (lot.side === "LONG" && incomingSide === "SELL")
              slicePnl = (price - lot.price) * m;
            else if (lot.side === "SHORT" && incomingSide === "BUY")
              slicePnl = (lot.price - price) * m;
        ...
        const openPositions = Array.from(books.values()).filter(
          (lots) => lots.reduce((s, l) => s + l.qty, 0) !== 0
        ).length;
        ...
        res
          .status(getStatus(e))
          .json({ error: "summary_failed", detail: getDetail(e) });
      }
    }) as RequestHandler
  );

  /** ---------- /api/strategies/summary (exec log collection) ---------- */
  app.get(
    ...
    (async (req: Request, res: Response) => {
      try {
        ...
        const strategyNameFilter = req.query.strategy
        ...
        const closeLots = (
          ...
          while (qtyToClose > 0 && fromLots.length > 0) {
            ...
            const pnlPerUnit = isClosingLong
              ? exitPrice - entry
              : entry - exitPrice;
        ...
        for (const [, stream] of byStream.entries()) {
          ...
          for (const e of stream) {
            ...
            if (side === "BUY") {
              ...
              const c2 = closeLots(
                shortLots,
                shortLots.reduce((a, l) => a + l.qty, 0),
                price,
                false
              );
        ...
        const data = Array.from(byStrategy.entries())
          .map(([strategyName, m]) => {
  ...
  _db
    .collection("tv_signal_tags")
    .createIndex({ userId: 1, orderTag: 1 }, { unique: true })
    .catch(() => {});
  _db
    .collection("tv_signal_tags")
    .createIndex({ userId: 1, strategyName: 1 })
    .catch(() => {});

  /** ---------- GET /api/summary/history (user-scoped) ---------- */
  app.get(
    "/api/summary/history",
    (async (req, res) => {
      try {
  ...
  /** ---------- GET /api/summary/by-date (user-scoped) ---------- */
  app.get(
    "/api/summary/by-date",
    (async (req, res) => {
      try {
        ...
        if (!dateKey)
          return res.status(400).json({ ok: false, error: "Missing dateKey" });

        const coll = _db.collection("api_summary");
        const doc = await coll.findOne(
          { userId: userId || null, dateKey },
          { projection: { _id: 0 } }
        );
  ...
  app.get(
    ...
    (async (req, res) => {
      try {
        ...
        const algoRows = rows.filter((x) =>
          String(x?.ordertag ?? "").startsWith(ALGO_PREFIX)
        );
  ...
  pnlColl
    .createIndex({ userId: 1, sliceKey: 1 }, { unique: true })
    .catch(() => {});
        ...
        const events: Ev[] = rows
          ...
          .map((r) => {
            ...
            return {
              ...
              uid: obRowUID(r),
              updatetime: String(
                r?.updatetime ?? r?.exchorderupdatetime ?? r?.exchtime ?? ""
              ),
  ...
  // ========================================================================
  // /api/strategies/pnl — join tv_signal_tags (by orderTag only) + user’s OB
  // ========================================================================
  app.get(
    "/api/strategies/pnl",
    (async (req: Request, res: Response) => {
      try {
        ...
        const strategyParam = (
          req.query.strategy ? String(req.query.strategy) : ""
        ).trim();
        ...
        if (!allTags.length) return res.json({ ok: true, data: [] });

        const obRows = await obColl
          .find(
        ...
        for (const r of obRows) {
          ...
          const qty = toNum(
            r.filledshares ?? r.filledqty ?? r.quantity ?? r.qty ?? 0
          );
        ...
        for (const [strategyName, rows] of byStrategy.entries()) {
          ...
          const rnr = sumLoss > 0 ? sumWin / sumLoss : null;
        ...
        if (!strategies) {
          ...
          data = out.sort(
            (a, b) =>
              (orderMap.get(a.strategyName) ?? 9999) -
              (orderMap.get(b.strategyName) ?? 9999)
          );
        }
  ...
  // --------------------------------------------------------------------------------------------
  // ===== List triggered trades from orderbook_raw (user-scoped, date-range) =====
  const buildTriggeredList = async (db: Db, userId: string, from: Date, to: Date) => {
    const fromMs = from.getTime();
    const toMs = to.getTime();

    const obColl = db.collection("orderbook_raw");

    const rows = await obColl
      .find(
        { userId, ordertag: { $regex: /^TV_/ } },
        {
          projection: {
            _id: 0,
            uniqueorderid: 1,
    ...
    const list = rows
      .filter((r) =>
        String(r.status ?? r.orderstatus ?? "").toLowerCase().includes("complete")
      )
      .map((r) => {
        const sideRaw = String(r.transactiontype ?? r.side ?? "").toUpperCase();
        const side: "BUY" | "SELL" = sideRaw === "SELL" ? "SELL" : "BUY";
        ...
        const price = obPreferFillPrice(r);
        ...
        const t = obParseTime({ updatetime: tsStr });

        const symbol = String(r.tradingsymbol ?? r.symbol ?? "").toUpperCase();
        const uid = obRowUID(r);
        const tag = String(r.ordertag ?? "");

        return { uid, t, symbol, side, qty, price, tag };
      })
      .filter(
        (x) => x.qty > 0 && Number.isFinite(x.price) && Number.isFinite(x.t)
      )
      .filter((x) => x.t >= fromMs && x.t <= toMs)
      .sort((a, b) => b.t - a.t);
    ...
    (async (req, res) => {
      try {
        ...
        if (!from || !to)
          return res
            .status(400)
            .json({ ok: false, error: "Missing from/to (ISO)" });

        const data = await buildTriggeredList(_db, userId, from, to);
        res.json({ ok: true, data });
      } catch (e: any) {
        res.status(500).json({ ok: false, error: e?.message || "Server error" });
      }
  ...
  // Public: legacy alias expected by your frontend
  app.get(
  ...
  // ===== Open positions from orderbook_raw (user-scoped, date-range) =====
  app.get(
    "/api/positions/open",
    (async (req, res) => {
      try {
        ...
        const events: Ev[] = rows
          ...
          .map((r) => {
            ...
            const price = obPreferFillPrice(r);
            const t = obParseTime(r);
            const symbol = String(r.tradingsymbol ?? r.symbol ?? "").toUpperCase();
            return { symbol, side, qty, price, t };
          })
          .filter(
            (x) => x.qty > 0 && Number.isFinite(x.price) && Number.isFinite(x.t)
          )
        ...
        type Lot = { qty: number; price: number; t: number };
        const longs = new Map<string, Lot[]>();
        const shorts = new Map<string, Lot[]>();

        const closeLots = (book: Lot[], qtyToClose: number) => {
        ...
        for (const e of events) {
          if (e.side === "BUY") {
            const book = shorts.get(e.symbol) ?? [];
            let remain = closeLots(book, e.qty);
            shorts.set(e.symbol, book);

            if (remain > 0) {
              const lb = longs.get(e.symbol) ?? [];
              lb.push({ qty: remain, price: e.price, t: e.t });
              longs.set(e.symbol, lb);
            }
          } else {
            const book = longs.get(e.symbol) ?? [];
            let remain = closeLots(book, e.qty);
            longs.set(e.symbol, book);

            if (remain > 0) {
              const sb = shorts.get(e.symbol) ?? [];
              sb.push({ qty: remain, price: e.price, t: e.t });
              shorts.set(e.symbol, sb);
            }
          }
        }
        ...
        const pushAgg = (
          ...
          if (qty > 0) {
            out.push({
              ...
              side,
              qty,
              avgEntry: Number((notional / qty).toFixed(2)),
              lastFillMs: lastFill,
            });
          }
        };

        const symbols = new Set([...longs.keys(), ...shorts.keys()]);
        for (const sym of symbols) {
          pushAgg(sym, "LONG", longs.get(sym) ?? []);
          pushAgg(sym, "SHORT", shorts.get(sym) ?? []);
        }

        out.sort((a, b) => b.lastFillMs - a.lastFillMs);
