// src/routes/advdec.routes.ts
import { Express, Request, Response } from 'express';
import { Db } from 'mongodb';

export function AdvDec(app: Express, db: Db) {
  app.get("/api/advdec", async (_req: Request, res: Response): Promise<void> => {
    try {
      const now = new Date();
      const marketOpen = new Date(
        now.getFullYear(), now.getMonth(), now.getDate(), 9, 15, 0, 0
      );
      const marketClose = new Date(
        now.getFullYear(), now.getMonth(), now.getDate(), 15, 30, 59, 999
      );
