...
function deriveUnderlyingSymbol(instr: any): string {
  ...
  if (c) return c.split(" ")[0]; // first token from display name
...
router.get("/underlyings", (req: Request, res: Response) => {
  ...
  const instrument = (req.query.instrument as string || req.query.instrumentType as string || "").trim().toUpperCase();

  let matches = instruments.filter(instr => {
    return (
      (!exchange || (instr.SEM_EXM_EXCH_ID || "").toUpperCase() === exchange) &&
  ...
  const list = Array.from(
    new Set(
      matches.map(x => deriveUnderlyingSymbol(x)).filter(Boolean)
    )
  ).sort();
...
router.get("/expiries", (req: Request, res: Response) => {
  ...
  const exchange = (req.query.exchange as string || "").trim().toUpperCase();
  const instrumentType = (req.query.instrumentType as string || "").trim().toUpperCase();
  const optionType = (req.query.optionType as string || "").trim().toUpperCase();
...
const searchHandler: RequestHandler = (req: Request, res: Response) => {
  ...
  let matches = instruments.filter(instr => {
    ...
    if (otypeMatch)
      match = match && (instr.SEM_OPTION_TYPE && (instr.SEM_OPTION_TYPE as string).toUpperCase() === otypeMatch);

    if (exchange)
      match = match && (instr.SEM_EXM_EXCH_ID && (instr.SEM_EXM_EXCH_ID as string).toUpperCase() === exchange);

    if (instrumentType)
      match = match && (instr.SEM_INSTRUMENT_NAME && (instr.SEM_INSTRUMENT_NAME as string).toUpperCase() === instrumentType);
