...
ws.on("close", (code, reason) => console.log(`❌ WebSocket closed. Code: ${code}, Reason: ${reason.toString()}`));
// --------------------
// WebSocket Subscription
// --------------------
function subscribeTicker(ids) {
    ...
    setInterval(async () => {
        try {
            for (let i = 0; i < securityIds.length; i += MAX_REST_BATCH) {
                ...
                console.log(JSON.stringify({ data: { [EXCHANGE_SEGMENT]: marketData }, status: "success" }, null, 2));
                // Store the data in MongoDB
                try {
                    if (Object.keys(marketData).length > 0) {
                        const timestamp = getISTTimestamp();
                        // Convert the market data to individual documents
                        const documents = Object.entries(marketData).map(([security_id, data]) => ({
                            security_id,
...
// --------------------
async function parseBinaryPacket(buffer) {
    ...
    switch (feedResponseCode) {
        case 2: {
            ...
            try {
                await collection.updateOne({ security_id: securityId }, {
                    $set: {
                        last_price: ltp,
