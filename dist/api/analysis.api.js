...
function AnalysisRoutes(app, db) {
    ...
    app.get("/api/net-oi", async (req, res) => {
        try {
            ...
            nseData.forEach(row => {
                ...
                if (!raw)
                    return; // or `continue` if you’re in a `for`‌loop
                const date = raw;
                ...
                const indexFutures = (row["Future Index Long"] || 0) - (row["Future Index Short"] || 0);
                const indexOptions = ((row["Option Index Call Long"] || 0) - (row["Option Index Call Short"] || 0)) -
            ...
            const mergedData = Object.values(netOIMap).sort((a, b) => {
                ...
                (0, dayjs_1.default)(a.date, "DD-MM-YYYY").toDate().getTime() - (0, dayjs_1.default)(b.date, "DD-MM-YYYY").toDate().getTime());
            });
            res.json(mergedData);
        }
        catch (err) {
