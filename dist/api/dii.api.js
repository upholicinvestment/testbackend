...
exports.default = DIIRoutes;
const dayjs_1 = __importDefault(require("dayjs"));
function DIIRoutes(app, db) {
    ...
    app.get("/api/DII_Index_Opt/data", async (req, res) => {
        try {
            ...
            const result = rows.map(row => ({
                Date: row.Date,
                DII_Call_Change: (row["Option Index Call Long"] || 0) - (row["Option Index Call Short"] || 0),
                DII_Put_Change: (row["Option Index Put Long"] || 0) - (row["Option Index Put Short"] || 0),
    ...
    app.get("/api/OIDII_Index_Opt/data", async (req, res) => {
        try {
            ...
            const result = rows.map(row => ({
                Date: row.Date,
                DII_Call_OI: (row["Option Index Call Long"] || 0) - (row["Option Index Call Short"] || 0),
                DII_Put_OI: (row["Option Index Put Long"] || 0) - (row["Option Index Put Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
        }
    ...
    app.get("/api/OIDII_Index_Fut/data", async (req, res) => {
        try {
            ...
            const result = rows.map(row => ({
                Date: row.Date,
                DII_Futures_OI: (row["Future Index Long"] || 0) - (row["Future Index Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
        }
