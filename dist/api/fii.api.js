...
exports.default = FIIRoutes;
const dayjs_1 = __importDefault(require("dayjs"));
function FIIRoutes(app, db) {
    ...
    app.get("/api/data", async (req, res) => {
        try {
            ...
            niftyRows.forEach(nifty => {
                const norm = (0, dayjs_1.default)(nifty.Date, "DD-MMM-YYYY").format("YYYY-MM-DD");
                niftyMap[norm] = Number(nifty.Close);
            });
            // Compute FII Call/Put changes and merge Nifty values
            const result = nseRows.map(row => ({
                Date: row.Date,
                FII_Call_Change: (row["Option Index Call Long"] || 0) - (row["Option Index Call Short"] || 0),
                FII_Put_Change: (row["Option Index Put Long"] || 0) - (row["Option Index Put Short"] || 0),
    ...
    app.get("/api/FII_Index_Fut/data", async (req, res) => {
        try {
            ...
            const resultWithChange = rows.map(row => {
                ...
                const currentNetOI = (row["Future Index Long"] || 0) - (row["Future Index Short"] || 0);
                const change = dateStr === "2025-04-07"
    ...
    app.get("/api/OIFII_Index_Opt/data", async (req, res) => {
        try {
            ...
            const result = rows.map(row => ({
                Date: row.Date,
                FII_Call_OI: (row["Option Index Call Long"] || 0) - (row["Option Index Call Short"] || 0),
                FII_Put_OI: (row["Option Index Put Long"] || 0) - (row["Option Index Put Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
        }
    ...
    app.get("/api/OIFII_Index_Fut/data", async (req, res) => {
        try {
            ...
            const result = rows.map(row => ({
                Date: row.Date,
                FII_Futures_OI: (row["Future Index Long"] || 0) - (row["Future Index Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
        }
