...
exports.default = registerNiftyRoutes;
function registerNiftyRoutes(app, db) {
    app.get('/api/nifty/atm-strikes-timeline', async (req, res) => {
        try {
            const intervalParam = req.query.interval || '3';
    ...
    app.get('/api/nifty/near5', async (_req, res) => {
        try {
            ...
            const results = await Promise.all(strikePrices.map(async (strike) => {
                ...
                CE_docs.forEach(doc => {
                    ...
                    if (!groupedByTimestamp[ts])
                        groupedByTimestamp[ts] = { strike_price: strike };
