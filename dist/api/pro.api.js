...
exports.default = ProRoutes;
const dayjs_1 = __importDefault(require("dayjs"));
function ProRoutes(app, db) {
    ...
    app.get("/api/Pro_Index_Opt/data", async (req, res) => {
        try {
            ...
            const result = rows.map((row) => ({
                Date: row.Date,
                Pro_Call_Change: (row["Option Index Call Long"] || 0) -
                    (row["Option Index Call Short"] || 0),
                Pro_Put_Change: (row["Option Index Put Long"] || 0) -
    ...
    app.get("/api/OIPro_Index_Opt/data", async (req, res) => {
        try {
            ...
            const result = rows.map((row) => ({
                Date: row.Date,
                Pro_Call_OI: (row["Option Index Call Long"] || 0) -
                    (row["Option Index Call Short"] || 0),
                Pro_Put_OI: (row["Option Index Put Long"] || 0) -
                    (row["Option Index Put Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
    ...
    app.get("/api/OIPro_Index_Fut/data", async (req, res) => {
        try {
            ...
            const result = rows.map((row) => ({
                Date: row.Date,
                Pro_Futures_OI: (row["Future Index Long"] || 0) - (row["Future Index Short"] || 0),
                NIFTY_Value: niftyMap[row.Date] ?? null,
            }));
            res.json(result);
        }
