...
exports.tradeJournalUpload = exports.standardGood = exports.standardDemons = void 0;
exports.parseUniversalTradebook = parseUniversalTradebook;
exports.processTrades = processTrades;
const multer_1 = __importDefault(require("multer"));
const csv_parser_1 = __importDefault(require("csv-parser"));
const fs_1 = __importDefault(require("fs"));
// ----------- CONSTANTS -----------
exports.standardDemons = [
...
// ----------- CSV PARSER -----------
function findTradeTableHeaderIndex(lines) {
    const possibleHeaders = [
        "Scrip/Contract,Buy/Sell,Buy Price",
        "symbol,isin,trade_date",
        "Scrip Name,Trade Type,Trade Date"
    ];
    return lines.findIndex(line => possibleHeaders.some(header => line.replace(/\s/g, '').toLowerCase().startsWith(header.replace(/\s/g, '').toLowerCase())));
}
async function parseUniversalTradebook(filePath) {
    const raw = fs_1.default.readFileSync(filePath, "utf-8");
    const lines = raw.split(/\r?\n/);
    const headerIdx = findTradeTableHeaderIndex(lines);
    if (headerIdx === -1)
        throw new Error("No recognizable trade table found!");
    const tradeTable = lines.slice(headerIdx).join("\n");
    const tempCsv = filePath + ".parsed.csv";
    fs_1.default.writeFileSync(tempCsv, tradeTable);
    return new Promise((resolve, reject) => {
        const trades = [];
        fs_1.default.createReadStream(tempCsv)
            ...
            // Angel/Upstox/ICICI-like
                ...
                const charges = chargeFields.reduce((sum, key) => sum + (parseFloat(row[key]) || 0), 0);
                trades.push({
                    Date: normalizeTradeDate(date),
                    Time: time,
            ...
            .on("end", () => {
            fs_1.default.unlinkSync(tempCsv);
            resolve(trades);
        })
            .on("error", (err) => {
            fs_1.default.unlinkSync(tempCsv);
...
// ----------- ROUND TRIP PAIRING -----------
function pairRoundTrips(trades) {
    const sorted = trades
    ...
    for (const trade of sorted) {
        ...
        if (openLegs.length > 0 &&
            ...
            while (openLegs.length && qtyToClose > 0) {
                ...
                const entryLegUsed = { ...entryLeg, Quantity: closeQty };
                ...
                const exitLeg = { ...trade, Quantity: closeQty };
                const pnl = entryLegUsed.Direction === "Buy"
...
function processTrades(trades) {
    ...
    let prevLossExitTime = null;
    let prevLossDirection = null;
    for (const rt of roundTrips) {
        netPnl += rt.PnL;
        const tradeDate = rt.exit.Date;
        pnlByDate[tradeDate] = (pnlByDate[tradeDate] || 0) + rt.PnL;
        if (!tradeDates.includes(tradeDate))
    ...
    const badTagSummary = {};
    const goodTagSummary = {};
    exports.standardDemons.forEach(tag => badTagSummary[tag] = { count: 0, totalCost: 0 });
    exports.standardGood.forEach(tag => goodTagSummary[tag] = { count: 0, totalProfit: 0 });
    ...
    for (let i = 0; i < roundTrips.length; i++) {
        ...
        if (t.PnL > 0 && t.entry.stopDistance && t.entry.stopDistance > 0) {
            ...
            if (rr < minGoodRR)
                demons.push("POOR RISK/REWARD TRADE");
        }
        if (t.PnL < 0 && t.holdingMinutes > 90)
            demons.push("HELD LOSS TOO LONG");
        if (t.PnL > 0 && t.holdingMinutes < 8 && t.PnL < avgWin * 0.8)
            demons.push("PREMATURE EXIT");
        if (t.PnL < 0 && Math.abs(t.PnL) > Math.abs(avgLoss * SLTolerance))
            demons.push("MISSED STOP LOSS");
        ...
        if (tradeNumberToday > overtradeLimit)
            demons.push("OVERTRADING");
    ...
    if (tradeWinPercent < 40)
        candidateActions.push("Trade only A+ setups for a week; skip marginal conditions.");
    if (!Number.isFinite(profitFactor) || profitFactor < 1)
        candidateActions.push("Tighten losses and let winners run using R-based take-profits.");
    if (enteredTooSoonCount > 0)
        candidateActions.push("No entries before 09:20—let structure form before engaging.");
    ...
    const upholicScore = Math.min(100, (patienceScore * 0.4) + (tradeWinPercent * 0.3) + (dayWinPercent * 0.3));
    return {
        netPnl,
        tradeWinPercent,
        profitFactor,
        ...
        standardDemons: exports.standardDemons,
        standardGood: exports.standardGood,
        enteredTooSoonCount
    };
}
