...
const oidFrom = (ns) => {
    const hex = crypto_1.default.createHash("md5").update(ns).digest("hex").slice(0, 24);
...
(async () => {
    ...
    try {
        ...
        await db.collection("products").bulkWrite(products.map((doc) => ({
            replaceOne: {
                filter: { _id: doc._id },
                replacement: doc,
                upsert: true,
        ...
        await db.collection("product_variants").bulkWrite(variants.map((doc) => ({
            replaceOne: {
                filter: { _id: doc._id },
                replacement: doc,
                upsert: true,
